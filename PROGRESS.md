...
_Last updated: 2026-08-20 — WORKLOG groomed: resolved + superseded entries moved here (see the last section), leaving 16 open items there. Nothing shipped on 2026-08-20; it was a backlog session. Previously: the whole prioritised WORKLOG except #8 is DONE and on `master`: #1 delete authz, #2 preset embedding lifecycle, #3 profile, #4 FE/BE flags, #5 disable+reassign, #6 finalise→preset library, #7 preset creation. Plus Tavily grounding, preset consolidation to one dev figure, and sequence-allocated preset codes. 284 unit tests. See the two sections below._
...
# Resolved WORKLOG entries — original analysis

Moved out of WORKLOG.md on 2026-08-20 so the backlog holds only open work.

These are the request write-ups as they stood *before* the work was done, each
with a status line recording what actually happened. They're kept in full rather
than summarised for two reasons: several diagnosed real bugs that the
implementation then confirmed (the `deleteEstimate` authorization hole, the
`beHours * 0.4` fabrication, `savePreset` silently de-indexing presets), and one
was **superseded** rather than built — the reasoning for going the other way is
only legible next to the original proposal.

The narrative of what shipped is in the dated sections above; this is the
archaeology.
